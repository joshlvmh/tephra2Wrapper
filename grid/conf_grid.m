...
if nargout == 1
    ...
    grid.min_east    = 622000;      % Minimum easting (UTM, WGS84)
    grid.max_east    = 707000;      % Maximum easting (UTM, WGS84)
    grid.min_north   = 3463000;     % Minimum northing (UTM, WGS84)
    grid.max_north   = 3530000;     % Maximum norting (UTM, WGS84)
    grid.res         = 3000;        % Grid resolution (m)
    grid.vent_zone   = 52;          % Vent UTM zone - only number, negative in S hemisphere
    grid.elevation   = 1;           % Mean grid elevation (m asl)
    grid.cross_zn    = 0;           % Sets if grid crosses multiple UTM zones
    grid.cross_eq    = 0;           % Sets if grid crosses the equator
    % If both cross_zn and cross_eq are 0
    grid.zone        = 52;          % UTM zone
    % If grid is across lateral zones - cross_zn == 1
    grid.zone_W      = [];          % UTM zone of the west grid
    grid.zone_E      = [];          % UTM zone of the east grid
    % If grid is across equator - cross_eq == 1
